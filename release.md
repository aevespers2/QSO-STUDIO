...
Status: `BLOCKED — DOCUMENTATION AND SYNTHETIC CONFORMANCE CANDIDATES IN REVIEW; PRODUCT CHARTER AND PUBLICATION APPROVAL REQUIRED`

QSO-STUDIO has a bounded documentation candidate and two current-main synthetic consumers: an independently implemented QSO-FABRIC manifest consumer and an independently implemented QSO Field architecture-review consumer. The documentation includes a truthful front door, product and UX charter, architecture, accessible diagrams, read-only review workflow, integration map, onboarding, developer guidance, accessibility and security/privacy requirements, punch list, deterministic checks, and hostile regressions. Neither consumer is **an authorized Pages publication, application, package, ecosystem admission, interface acceptance, reviewer registry, architecture authority, or release**.
...
- No version may imply a site, executable Studio, admitted component, accepted interface, reviewer appointment, real quorum, architecture decision, orchestration authority, repository authority, or payment capability that is not present and verified.
...
- Accessibility, security, privacy, correction, withdrawal, supersession, and rollback expectations.
...
| Provenance | REVIEW | Exact-head workflows must retain source identity, deterministic input hashes, tests, and artifact metadata. |
| Approval | PENDING | Separate charter, architecture, publication, release, and later implementation decisions are required. |
...
Withdraw the documentation or either consumer candidate when it makes an unsupported publication, capability, admission, appointment, quorum, decision, or compatibility claim; merges proposal and authority states; loses source provenance; binds a moved producer tuple without supersession; hides a correction or withdrawal; introduces inaccessible navigation or diagrams; includes restricted data; or cannot be reproduced. Restore the prior verified documentation state, mark affected routes stale or withdrawn, preserve failed candidate evidence, and require a new exact-head generation.
...
- Documentation, accessibility, privacy, security, integration, publication, evidence, and rollback ownership.
